...
def health() -> dict[str, Any]:
    ...
    catalog_stats = services.catalog.stats() if services.catalog else {"products": 0}
    kb_ready = rag_stats.get("documents", 0) > 0 and catalog_stats.get("products", 0) > 0
    return {
        "status": "ok",
...
def catalog_stats() -> dict[str, Any]:
    ...
    if not services.catalog:
        raise HTTPException(status_code=503, detail="Mixed agricultural catalog is unavailable")
    return services.catalog.stats()
...
def catalog_products(
    ...
    if not services.catalog:
        raise HTTPException(status_code=503, detail="Mixed agricultural catalog is unavailable")
    products = services.catalog.search_products(
        query,
...
def catalog_product_detail(
    product_id: str, include_synthetic: bool = Query(default=False)
) -> dict[str, Any]:
    services = get_services()
    if not services.catalog:
        raise HTTPException(status_code=503, detail="Mixed agricultural catalog is unavailable")
    product = services.catalog.get_product(
        product_id, include_synthetic=include_synthetic
    )
    if not product:
        raise HTTPException(status_code=404, detail="Product not found under the current safety filter")
    return product
...
def kb_stats() -> dict[str, Any]:
    ...
    stats["data_policy"] = {
        ...
        "authentic_mixed_catalog_60": "Authentic source-derived identities, varieties, agronomy, and contextual fertilizer evidence from the integrated catalog.",
        "synthetic_mixed_catalog_40": "Explicitly fictional test entities; hidden by default and blocked from planning or prescriptive advice.",
